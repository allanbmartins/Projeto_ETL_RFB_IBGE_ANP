...
def descompactar_arq_rais():
    ...
    try:
        ...
        Files = list(filter(lambda name: nome_arquivo in name,
                     os.listdir(output_files)))

        print_divisor_inicio_fim('Arquivos da RAIS que serão descompactados:',
                                 1)
...
def converter_utf8_arq_rais():
    ...
    try:
        ...
        Items = list(filter(lambda name: '' in name,
                     os.listdir(extracted_files)))

        # Verifica se a lista de Items é vazia
        if len(Items) != 0:
            print_divisor_inicio_fim('Arquivos da RAIS que serão convertidos e separados caso necessário: ',
                                     1)
            ...
            print_divisor_inicio_fim('!!! ATENÇÃO NÃO TEM ARQUIVOS NA PASTA ESPECÍFICADA !!!',
                                     3)

            logs.record('!!! ATENÇÃO NÃO TEM ARQUIVOS NA PASTA ESPECÍFICADA !!!',
                        colorize=True)
...
def inserir_dados_rais_bd():
    ...
    try:
        ...
        leitura_csv_insercao_bd_sql('Estb2021ID',
        ...
        print_parcial_final_log_inf_retorno(f'inserção no banco de dados dos dados do RAIS',
                                            insert_start,
                                            insert_end,
                                            '',
                                            'geral')
...
def dados_faltantes():
    ...
    try:
        ...
        def faltantes_estabelecimentos():
            ...
            output_erros = (os.path.join(GetEnv('RAIS_OUTPUT_ERROS_PATH'),
...
def sequencia_rais():
    ...
    try:
        ...
        funçao_barprogress([descompactar_arq_rais,
        ...
        print_parcial_final_log_inf_retorno(f'converter e inserção no banco, remoção de cnpj duplicados e crição de chaves primárias e estrangeiras nas tabelas do RAIS na {base_dados}',
                                            insert_start,
                                            insert_end,
                                            '',
                                            'geral')
